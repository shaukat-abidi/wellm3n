...
void my_read_input_parameters(int argc, char* argv[], char *groundtruth_file, char *trainfile, char *modelfile,char *selected_supervision_file,
...
SVECTOR* find_cutting_plane(EXAMPLE *ex, SVECTOR **fycache, double *margin, long m, STRUCTMODEL *sm, STRUCT_LEARN_PARM *sparm) {
  ...
  /*
  printf("inside find_cutting_plane\n");
...
double cutting_plane_algorithm_copy_from_Massimo_package(double *w, long m, int MAX_ITER, double C, double epsilon, SVECTOR **fycache, EXAMPLE *ex, STRUCTMODEL *sm, STRUCT_LEARN_PARM *sparm) {
  long i,j;
  double xi;
  double *alpha;
...
#if (DEBUG_LEVEL>0)
    for (j=0;j<size_active;j++) {
      printf("alpha[%ld]: %.8g, cut_error[%ld]: %.8g\n", j, alpha[j], j, cut_error[j]);
    }
    printf("sigma_k: %.8g\n", sigma_k); 
    printf("alphasum: %.8g\n", alphasum);
    printf("g^T d: %.8g\n", gTd); 
...
double cutting_plane_algorithm(double *w, long m, int MAX_ITER, double C, double epsilon, SVECTOR **fycache, EXAMPLE *ex, STRUCTMODEL *sm, STRUCT_LEARN_PARM *sparm) {
  ...
  while ((!suff_decrease_cond)&&(expected_descent<-epsilon)&&(iter<MAX_ITER)) { 
    ...
    for (iter_w = 1; iter_w <= sm->sizePsi; iter_w++)
    {
    printf ("InsideInnerLoop  w[%ld]=%lf w_b[%ld]=%lf\n",iter_w, w[iter_w],iter_w, w_b[iter_w]);
    }
    */ 
...
int main(int argc, char* argv[]) {
  ...
  char trainfile[1024];
  char modelfile[1024];
  char selected_supervision_file[1024];
  char groundtruth_file[1024];
  ...
  EXAMPLE *ex;
  SAMPLE sample, supervised_sample_buffer;
  ...
  long lot,iter_entries,ex_num,tok_index;

  /* read input parameters */
  my_read_input_parameters(argc, argv, groundtruth_file, trainfile, modelfile, selected_supervision_file, &learn_parm, &kernel_parm, &sparm);
  ...
  printf("Reading file for selecting groundtruth labels ... : %s\n\n",selected_supervision_file);
  
  epsilon = learn_parm.eps;
  C = learn_parm.svm_c;
  MAX_ITER = learn_parm.maxiter;

  /* read in examples and supervised samples */
  sample = read_struct_examples(trainfile,&sparm);
  /* Read groundtruth file */ 
  sparm.supervised_sample = read_struct_examples(groundtruth_file,&sparm); 
  supervised_sample_buffer = read_struct_examples(groundtruth_file,&sparm); 
  ex = sample.examples;
  m = sample.n;
  ...
  {
	  lot = sparm.supervised_sample.examples[iter_ex].y.length;
	  for(iter_tok=0;iter_tok<lot;iter_tok++){
		  /* change all labels to zero */
		  sparm.supervised_sample.examples[iter_ex].y.label_vector[iter_tok]=0;
	  }
  }
  ...
  {
	ex_num = sup_labels.seq_id[iter_entries] - 1; /* Was 1-based */
	tok_index = sup_labels.frame_id[iter_entries] - 1; /* Was 1-based */   
	sparm.supervised_sample.examples[ex_num].y.label_vector[tok_index] = supervised_sample_buffer.examples[ex_num].y.label_vector[tok_index];	  
  }
  
  /* Step-3: Print supervised dataset for verification */
  print_dataset(sparm.supervised_sample);
  ...
  //printf("sm.svm_model->sv_num: %d\n", sm.svm_model->sv_num);
  //printf("sm.svm_model->kernel_parm.kernel_type: %d\n", sm.svm_model->kernel_parm.kernel_type);
  ...
  /*	
  // Display value of hidden values (initial value)-------	
	int indexi;
	printf("\n Start display initial hidden value \n");
	for (i=0;i<m;i++) {      
	  for (indexi=0;indexi<(ex[i].x.length);indexi++){	
	  printf(" %d", ex[i].h.states[indexi]);
	  }printf("\n");	}
	  printf("\n End display initial hidden value \n\n\n");
  //-------------------------------------------------------	
  */	  
  ...
  while ((outer_iter<2)||((!stop_crit)&&(outer_iter<MAX_OUTER_ITER))) { 
    ...
    for (i=0;i<m;i++) {
      ...
      sparm.lot = ex[i].x.length;
      sparm.sample_index = i;
      //printf("sparm.sample_index:%ld,sparm.lot:%ld\n",sparm.sample_index,sparm.lot);
      
      /* Original */
      //ex[i].h = infer_latent_variables(ex[i].x, ex[i].y, &sm, &sparm);
	  
       ex[i].h = infer_latent_variables(ex[i].x, ex[i].y,sparm.supervised_sample.examples[i].y.label_vector, &sm, &sparm);

	  
	   //!!! display value of hidden variables after each interation 
	  /*
	  for (indexi = 0; indexi < (ex[i].x.length); indexi++){	
	  printf(" %ld", ex[i].h.states[indexi]);}
	  
	  printf("\n");	
	  */
	  
	  /*Copy it to sparm*/
	  for(iter_tok=0;iter_tok<sparm.lot;iter_tok++){
	  	  sparm.h_init[i].states[iter_tok] = ex[i].h.states[iter_tok];
	  }
...
void my_read_input_parameters(int argc, char *argv[], char *groundtruth_file, char *trainfile, char* modelfile,char *selected_supervision_file,
  ...
  if((i+1)<argc) {
    ...
    strcpy (selected_supervision_file, argv[i+2]);
    strcpy (groundtruth_file, argv[i+3]);
  }
