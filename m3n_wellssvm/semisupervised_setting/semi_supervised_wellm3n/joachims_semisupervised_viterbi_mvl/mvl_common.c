...
void read_input_parameters(int argc,char *argv[],char *real_groundtruth_file, char *selected_supervision_file)
{
  /* real_groundtruth_file: Real ground-truth training file used for labeling selected frames ONLY */
  /* selected_supervision_file: File having indices of supervised frames */
  	
  strcpy (real_groundtruth_file, argv[1]); 
  strcpy (selected_supervision_file, argv[2]); 
}
...
{
	long lot,iter_tok;
	lot = dataset.examples[ex_num].y.length;
	for(iter_tok=0;iter_tok<lot;iter_tok++){
		//printf("%ld qid:%ld tok:%ld\n",dataset.examples[ex_num].y.labels[iter_tok],ex_num,iter_tok);
		/* To be consistent with 1-based index system, so that we can match it with data file in GEANY*/
		printf("%ld qid:%ld tok:%ld\n",dataset.examples[ex_num].y.labels[iter_tok],ex_num+1,iter_tok+1);
	}
}
...
void get_best_labels(long *labels,double *r,long *eb,long *tb,long lot,long sample_id)
{
	...
	/* It contains joachims implementation of Viterbi Algorithm */
	/* FOR-loop variables */
	long iter_ex,iter_tok,iter_states;
	
	printf("Calling viterbi \n");
	printf("ex: %ld , lot: %ld\n",sample_id,lot);
	printf("emit base: %ld , trans_base: %ld\n",eb[sample_id],tb[sample_id]);
	...
	/* Generate emit and trans matrix */
	/* emit[lot - 1][2] = tot_tokens x 2 matrix */
	/* emit[t][0] = emission score for the 1st state of the t^{th} token */
	/* emit[t][1] = emission score for the 2nd state of the t^{th} token */
	/* trans[4 * (lot - 1)] = 4 x (tot_tokens - 1) vector */
	...
	trans=(double*)my_malloc(sizeof(double) * (4*(lot-1)) );
	
	for(iter_tok=0;iter_tok<lot;iter_tok++) {
    emit[iter_tok]=(double *)my_malloc(sizeof(double)*(states));
    }
	...
	for(iter_tok = 0;iter_tok<lot;iter_tok++)
	{  
		emit[iter_tok][0] = r[base_index];
		emit[iter_tok][1] = r[base_index + 1];
		base_index += 2; /* base-index for the next token inside r */
	}
	
	/* fill trans */
	/* Copy transition indices and weights*/
	ind_w_pp = tb[sample_id]; /*11*/
	ind_w_np = tb[sample_id]+2;/*21*/
	ind_w_pn = tb[sample_id]+1;/*12*/
	ind_w_nn = tb[sample_id]+3;/*22*/
	iter_trans = 0;
	for(iter_tok = 1;iter_tok<=(lot-1);iter_tok++)
	{
	    ...
	    /* populate trans */
	    /* trans = [w_pp,w_pn,w_np,w_nn...]*/
	    trans[iter_trans + 0] = w_pp;
	    trans[iter_trans + 1] = w_pn;
	    trans[iter_trans + 2] = w_np;
	    trans[iter_trans + 3] = w_nn;
	...
	/* Call Viterbi */
	//long *viterbi(double *trans, double **emit, long length, long states,long trans_order, long emit_order, double *score);
	viterbi_lables = viterbi(trans,emit,lot,states,1,0,&score);
    
    for(iter_tok=0;iter_tok<lot;iter_tok++)
    {labels[iter_tok] = viterbi_lables[iter_tok];}
    /* print score */ 
    printf("best score: %f ",score);   
...
void get_best_labels_single_sequence(long *labels,double *r,long eb,long tb,long lot)
{
	...
	/* labels(TBR) : contain labels obtained by an application of Viterbi algorithm              */
	/* r           : Hy+tow+delta 													             */
	/* eb          : Starting index of emission feature */
	/* tb          : Starting index of transition feature */
	/* lot         : length of token in ex_num(sample_id)			 		                     */
...
void get_best_labels_single_sequence_semisupervised(long *supervised_labels, long *labels,double *r,long eb,long tb,long lot)
{
	/* TBR         : to-be-returned                                                              */
	/* labels(TBR) : contain labels obtained by an application of Viterbi algorithm              */
	/* r           : Hy+tow+delta 													             */
	...
	viterbi_lables = viterbi_semisupervised(supervised_labels,trans,emit,lot,states,1,0,&score); /* For semi-supervised case*/ 
...
void get_best_labels_my_implementation_of_viterbi(long *labels,double *r,long *eb,long *tb,long lot,long sample_id)
{
	/* TBR         : to-be-returned                                                              */
	/* labels(TBR) : contain labels obtained by an application of Viterbi algorithm              */
	/* r           : Hy+tow+delta 													             */
...
void get_best_labels_semisupervised_last_state_under_modification(long *supervised_labels, long *labels,double *r,long *eb,long *tb,long lot,long sample_id)
{
	/* TBR         : to-be-returned                                                              */
	/* labels(TBR) : contain labels obtained by an application of Viterbi algorithm              */
	/* r           : Hy+tow+delta 													             */
	...
	/* lot         : length of token in ex_num(sample_id)			 		                     */
	/* sample_id   : current sample/example number   							                 */
	/* supervised_labels: contains labels for supervised frames; 0 means unsupervised. Its length is 
	...
	for(iter_tok = 1;iter_tok<lot;iter_tok++)
	{
		...
		for(iter_states=1;iter_states<=2;iter_states++)
		{
			if(iter_states==1)
			{
				...
				/* get label of best score from above expression */
				if( (supervised_labels[iter_tok-1] == 1) || (supervised_labels[iter_tok-1] == 2) ){
					best_label = supervised_labels[iter_tok-1];
				}
				else{
				get_max_label(&best_label,current_scores[1],current_scores[2]);
				}			
	...
	/* Final Step */
	if( (supervised_labels[lot-1] == 1) || (supervised_labels[lot-1] == 2) ){
		if( supervised_labels[lot-1] == 1 )
		{
			psi_p[lot-1] = 1;
			copy_long_array(psi_p,labels,lot);
		}
...
void get_best_labels_semisupervised(long *supervised_labels, long *labels,double *r,long *eb,long *tb,long lot,long sample_id)
{
	 ...
	 *                     equal to the length of labels.                                        */
						  ...
						  /***************************************/
						  /* Joachims Implementation of Viterbi */	
						  /**************************************/
	...
	/* Call Viterbi */
	//long *viterbi(double *trans, double **emit, long length, long states,long trans_order, long emit_order, double *score);
	//viterbi_lables = viterbi(trans,emit,lot,states,1,0,&score); /* for UNSUPERVISED CASE */
...
/****************************************************************************************************/
void get_best_labels_semisupervised_my_implementation_of_viterbi(long *supervised_labels, long *labels,double *r,long *eb,long *tb,long lot,long sample_id)
{
	/* TBR         : to-be-returned                                                              */
	/* labels(TBR) : contain labels obtained by an application of Viterbi algorithm              */
	/* r           : Hy+tow+delta 													             */
...
void write_labels_only_to_file(char *filename,long *labels,long lot)
{
	...
	for(iter_lot=0;iter_lot<lot;iter_lot++){
		fprintf(fp,"%ld\n",labels[iter_lot]);	
	}
	fclose(fp);
...
void print_dataset(SAMPLE sample_dataset)
{
  ...
  for(iter_ex=0;iter_ex<tot_ex;iter_ex++)
  {
	  ...
	  for(iter_tok=0;iter_tok<lot;iter_tok++){
		  printf("%ld qid:%ld\n",ex[iter_ex].y.labels[iter_tok],iter_ex+1);
	  }
  }
}
...

{
  ...
  long iter_trans,iter_tok,iter_states;
  ...
  /* Print */
  printf("From Viterbi: lot=%ld, states=%ld, trans_order=%ld, emit_order=%ld\n", length, states, trans_order, emit_order);
  ...
  /* print temp and emit */
  for(iter_trans=0;iter_trans<4*(length-1);iter_trans++){printf("trans[%ld]=%f\n",iter_trans,trans[iter_trans]);}
  for(iter_tok=0;iter_tok<length;iter_tok++){
	  printf("emit[%ld][%ld]=%f\n",iter_tok,0,emit[iter_tok][0]);
	  printf("emit[%ld][%ld]=%f\n",iter_tok,1,emit[iter_tok][1]);
  }
  
  /* Do forward pass of Viterbi. */ 
  //VPATH *viterbi_forward_order1(double *trans, double **emit, long emit_order, long start, long end, long states, VPATH **path);   
  ...
  *score = bestpathscore[length - 1]; /* the score for the whole sequence
  ...
  //printf("best path \n");
  //for(iter_tok=0;iter_tok<length;iter_tok++){
  //	  printf("%ld ",bestpathlabel[iter_tok]);
  //}
  //printf("\n");
  //printf("best score: %f ",*score);
	...
	tokens long. */
{
  ...
  /**/
  printf("from main viterbi: start:%ld end:%ld\n",start,end);
  /* We are working for two states only */
  states = 2;
  ...
  for(p = 0; p < states; p++) { /* source state, h_t-1 */
    current=path_location;
	emit_ptr=emit_location;
    //trans_ptr=&trans[p*states]; /* original line */
    trans_location = (2*p) + (4*(location-1));
    trans_ptr=&trans[trans_location]; /* altered weights for transition scores */
    //printf("location:%ld,p:%ld,trans_location:%ld\n",location,p,trans_location);
    for(si=0;si<states;si++) {  /* destination state, h_t */ 
