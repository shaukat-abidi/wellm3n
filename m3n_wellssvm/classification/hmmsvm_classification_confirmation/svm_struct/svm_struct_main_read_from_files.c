...
{  
  ...
  /**************************/
  long total_iterations,iter_labels;
  ...
  /*Read mu from file*/
  read_mu("mu.txt",&structmodel,structmodel.mu);
  ...
  /*Read labels*/
  /*
  gt_labels = (long *)my_malloc(sizeof(long) * sample.total_tokens);
  read_labels("labels.txt",gt_labels);
  for(iter_labels=0;iter_labels<sample.total_tokens;iter_labels++)
  {
	  printf("gt_labels[%ld] = %ld\n",iter_labels,gt_labels[iter_labels]);fflush(stdout);
  }
  */
