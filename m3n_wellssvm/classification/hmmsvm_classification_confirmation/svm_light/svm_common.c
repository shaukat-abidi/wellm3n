...
void scale_fvec(SVECTOR *fvec,float scale_factor)
{
	/* *******************************************************
	 * 
	 * Scales SVECTOR until it reaches NULL
	 * 
	 * *******************************************************
	 *
	 */
	SVECTOR *store_fvec_baseadd=NULL;
	store_fvec_baseadd=fvec;
	
    long iter_words = 0;
	
	while(fvec != NULL){
		iter_words=0;
		while(fvec->words[iter_words].wnum)
		{
		  fvec->words[iter_words].weight = scale_factor * fvec->words[iter_words].weight;
		  iter_words++;
		}
		fvec=fvec->next;
	}
	/* Restore base address for fvec */
	fvec = store_fvec_baseadd;
...
void print_fvec(SVECTOR *fvec)
{
	 ...
	 * 
	 * Debugging function:
	 * Prints passed SVECTOR till fvec->next == NULL   
	 * *******************************************************
	 *
	 */
	...
	while(debug_vec != NULL){
		...
		{
		  printf("iter_words = %ld wnum=%i , weight=%f ",iter_words,(int)debug_vec->words[iter_words].wnum,
     ...
     /*            not be larger than the weight vector!               */
{
  ...
  //print_fvec(ex->fvec);/*MS Adding*/
  //printf("totwords = %ld\n",model->totwords);/*MS Adding*/
  //printf("totdoc = %ld\n",model->totdoc);/*MS Adding*/
...
void scale_svector(SVECTOR *vec_s,float scale_factor)
{
	register WORD *ai;
	ai=vec_s->words;
	while(ai->wnum){
		ai->weight *= scale_factor;
		ai++;
	}
}
...
{
  //printf(" ---- free_model called with deep=%d----- \n",deep);
  long i;
