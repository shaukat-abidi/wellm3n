...
typedef struct label {
  ...
  long    **working_set_labels;
  long    *buffer_labels;/*buffer for labels. If it belongs to working set
						then we will copy them inside working_set_labels*/
  long    length;
...
typedef struct structmodel {
  ...
  /*Well-SSVM variables*/
  long sizePsi_atomic; /*sizePsi for one model*/
  long no_ol_ws;       /*number of labelings in working set*/
  long current_outer_iter; /* current outer iteration where mu's are changing */
  long iter_ex_ws;     /*iterate examples for working set */
  int L;              /*number of ground truth labels*/
  float *mu;         /*parameters containing weights for each slice*/
