...
{  
  ...
  /**************************/
  long i,total_iterations,iter_labels;
  long iter_tokens,iter_ex,T,iter_mu,iter_models;
  int alg_type,s_ind,e_ind,slice_index;//Start-End index
  double norm_full_w;
  double *norm_wi=NULL;
  long *gt_labels=NULL;
  ...
  /*Read L from File*/
  read_L("/home/ssabidi/Shaukat/WellSSVM/implementation/mkl/WellSSVM/hmmsvm/params/L.txt",&structmodel.L);
  T=structmodel.L;
  struct_parm.L=structmodel.L;  
  printf("structmodel.L=%d\n",structmodel.L);
  printf("struct_parm.L=%d\n",struct_parm.L);
  
  //printf("Abs: %f\n",fabs(-3.555));
  //exit(0);
  
  /*Allocating memory for mu and norm_wi*/
  structmodel.mu = (float *)my_malloc(sizeof(float) * structmodel.L);
  struct_parm.mu = (float *)my_malloc(sizeof(float) * structmodel.L);
  norm_wi = (double *)my_malloc(sizeof(double) * structmodel.L);
  
  /*Initialize norm_wi and mu*/
  for(iter_mu=0;iter_mu<structmodel.L;iter_mu++)
  {
	  /* mu and norm_wi have same number of instances */
	  structmodel.mu[iter_mu]=0.0;
	  struct_parm.mu[iter_mu]=0.0;
	  norm_wi[iter_mu] = 0.0;
  }
  
  /*Read mu from file*/
  read_mu("/home/ssabidi/Shaukat/WellSSVM/implementation/mkl/WellSSVM/hmmsvm/params/mu.txt",&structmodel,structmodel.mu);
  
  /* Store mu_s in structparm*/
  for(iter_models=0;iter_models<structmodel.L;iter_models++){
	   struct_parm.mu[iter_models] = structmodel.mu[iter_models];
  }
  
  /*Print mu_s*/
  for(iter_models=0;iter_models<structmodel.L;iter_models++)
  {
	  printf("structmodel.mu[%ld] = %f\n",iter_models,structmodel.mu[iter_models]);fflush(stdout);	  
  }
  /*Print mu_s*/
  for(iter_models=0;iter_models<struct_parm.L;iter_models++)
  {
	  printf("struct_parm.mu[%ld] = %f\n",iter_models,struct_parm.mu[iter_models]);fflush(stdout);
  }
  ...
  printf("ALG_TYPE: %d\n", alg_type);
  printf("changing ALG_TYPE to 3\n");
  alg_type = 3; /*one slack dual without cache*/
  printf("changed ALG_TYPE: %d\n", alg_type);
  printf("sample.n = %d\n",sample.n);
  printf("sample.tot_tokens = %ld\n",sample.total_tokens);
  
  
  /*Read files consecutively and store there labels*/
  SAMPLE *sample_to_delete=NULL;
  sample_to_delete = (SAMPLE *)my_malloc(sizeof(SAMPLE) * structmodel.L);
  char base_path[255] = "/home/ssabidi/Shaukat/WellSSVM/implementation/mkl/WellSSVM/hmmsvm/params/";
  char train_file[structmodel.L][255];
  
  //Generating text
  for(i=0;i<structmodel.L;i++)
  {
    snprintf(train_file[i],255,"%sgt_%ld.txt",base_path,i);
  }
  //Printing Text
  for(i=0;i<structmodel.L;i++)
  {
	  //Verifying train file
      printf("Reading %s ... \n",train_file[i]);fflush(stdout);
      sample_to_delete[i]=read_struct_examples(train_file[i],&struct_parm);
      printf("Done. \n");fflush(stdout);
          
  }
  //Populating Extended Labels Datastructure
  printf("Populating Extended Labels Datastructure... ");
  populate_extended_labels(&sample,&structmodel,sample_to_delete);
  printf("done.\n");
  
  /*Initializing buffer labels*/
  printf("Initializing buffer Labels... ");
  initialize_buffer_labels(&sample,&structmodel);
  printf("done.\n");
  
  //print_sample_info(&sample);
   
  /*Purge sample_to_delete*/
  for(i=0;i<structmodel.L;i++)
  {
	printf("freeing sample_to_delete[%ld]...\n",i);fflush(stdout);  
	free_struct_sample(sample_to_delete[i]);
  }
    

  /* Do the learning and return structmodel. */
  while(total_iterations<15)
  {
	  printf("*******starting iteration # %ld  .... ******\n",total_iterations);
	  structmodel.current_outer_iter = total_iterations;	
	  	  /*Print mu_s*/
			  for(iter_models=0;iter_models<structmodel.L;iter_models++)
			  {
				  printf("mu[%ld] = %f\n",iter_models,structmodel.mu[iter_models]);fflush(stdout);
			  ...
			  if(total_iterations < 1000){
				printf("calling with new model,  total_iterations %ld \n",total_iterations);fflush(stdout);
				svm_learn_struct_joint(sample,&struct_parm,&learn_parm,&kernel_parm,&structmodel,ONESLACK_DUAL_ALG); /* Main algo run by MS*/
				//svm_learn_struct_joint(sample,&struct_parm,&learn_parm,&kernel_parm,&structmodel,ONESLACK_PRIMAL_ALG); /* Different run by MS*/
			}
			  else{
				printf("calling with cached model,  total_iterations %ld \n",total_iterations);fflush(stdout);
  				//svm_learn_struct_joint(sample,&struct_parm,&learn_parm,&kernel_parm,&structmodel,ONESLACK_DUAL_CACHE_ALG);
				//svm_learn_struct_joint_for_iterations(sample,&struct_parm,&learn_parm,&kernel_parm,&structmodel,ONESLACK_DUAL_CACHE_ALG);
				//svm_learn_struct_joint_for_iterations(sample,&struct_parm,&learn_parm,&kernel_parm,&structmodel,ONESLACK_DUAL_ALG);
				svm_learn_struct_joint_for_iterations(sample,&struct_parm,&learn_parm,&kernel_parm,&structmodel,ONESLACK_DUAL_ALG);
			}
			  ...
			  norm_full_w = 0.0; /*set it to zero. It is the denominator of MU-Update Equation*/
			  ...
			  for(iter_models=0;iter_models<structmodel.L;iter_models++)
			  {
				  ...
				  printf("start_ind=%d,end_index=%d,norm(unscaled)=%f,",
				  s_ind,e_ind,norm_wi[iter_models]);fflush(stdout);
				  
				  /*Scale Norm by sqrt(mu)*/
				  norm_wi[iter_models] = norm_wi[iter_models] * sqrt(structmodel.mu[iter_models]);
				  printf("norm(scaled)=%f\n",norm_wi[iter_models]);fflush(stdout);
				  norm_full_w += norm_wi[iter_models];
			  }
			  printf("scaled norm_full_w = %f\n",norm_full_w);
			  
			  /*Add correction factor (Laplacian correction)*/
			  /*
			  for(iter_models=0;iter_models<structmodel.L;iter_models++)
			  {
				  if (structmodel.mu[iter_models] < 0.0001){
				  			norm_wi[iter_models] = 0.00001; 
				  			printf("correction factor added");
						}
			  }
			  */
			  ...
			  for(iter_models=0;iter_models<structmodel.L;iter_models++)
			  {
				  structmodel.mu[iter_models] = 0.0; /*Reset value*/
				  structmodel.mu[iter_models] = norm_wi[iter_models]/norm_full_w; /*Recompute it*/
				  
				  /*Assign it to struct_parm */
				  struct_parm.mu[iter_models] = structmodel.mu[iter_models]; 
			  }
  ...
  printf("no of gt labelings found in final working set: %ld \n",structmodel.no_ol_ws);
  ...
  /*Write learned mu's to file*/
  printf("Writing mu's...");fflush(stdout);
  write_mu_to_file("/home/ssabidi/Shaukat/WellSSVM/implementation/mkl/WellSSVM/hmmsvm/params/mu_converged.txt",&structmodel);
  printf("done\n");fflush(stdout);
