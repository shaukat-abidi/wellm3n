		      ...
		      STRUCTMODEL *sm, int alg_type)
{
  ...
  if(svmModel) {
    ...
    print_wvec(sm);/*MS: Adding it to print weight vector*/
  }
			    ...
			    STRUCTMODEL *sm, int alg_type)
{
  ...
  */
  ...
  /*MS: Augmenting Working Set*/
  long iter_ex,iter_ws;
  char filename_ws[255];
  ...
  init_struct_model(sample,sm,sparm,lparm,kparm); 
  /* Massimo-Shaukat: We have commented the bottom line as 
   * sizePsi has been initialized properly inside init_struct_model
   */
  //sizePsi=sm->sizePsi+1;          /* sm must contain size of psi on return */
  ...
  /* set initial model and slack variables */
  printf("**** malloc for svmModel ****** \n");
  svmModel=(MODEL *)my_malloc(sizeof(MODEL));
  lparm->epsilon_crit=epsilon;
  
  /*
  printf("svmModel->totwords = %ld\n",svmModel->totwords);
  printf("sizePsi = %ld\n",sizePsi);
  printf("svmModel->totdoc = %ld\n",svmModel->totdoc);
  printf("svmModel->totdoc = %ld\n",svmModel->totdoc);
  */
  ...
  for(i=0;i<n;i++) {
    ...
    //printf("---fycache----ex:%ld\n",i);fflush(stdout);/*MS-Adding*/
    //print_fvec(fycache[i]);/*MS-Adding*/
  ...
  do { /* iteratively find and add constraints to working set */
	...
	/* MS Adding this bit */
	printf(" We will see warning soon.... try setting violation to zero \n");
	//viol = 0.0;
	/* exit(1); */
      }
	/*****************ADDED by MS (End)****************************************************/
	...
	/* MS Adding this bit */
	printf("Setting slack to zero from here \n");
	/* exit(1); */
      }
	...
	/* MS:- Augmenting working set */
	if(sm->current_outer_iter == 14)
	{
		/*clear filename*/
		//filename_ws = "";
		/*generate filename*/
		snprintf(filename_ws,255,"/home/ssabidi/Shaukat/WellSSVM/repository/mkl/output/ws_%ld.txt",totconstraints-1);
		   
		//printf("augmenting working set,last_outer_iter...%ld\n",sm->current_outer_iter);
		
		/*write on file*/
		write_ws_to_file(filename_ws,&sample);   													   													   
	...
	for(j=0;j<cset.m;j++) 
	{
		...
		/**********/
		//printf(" alpha[%ld]=%f,alphahist[%ld]=%ld",j,alpha[j],j,alphahist[j]);   /*MS:Adding*/
		/**********/
	}
	
	if(sm->current_outer_iter == 14){
	   /*generate filename*/
	   snprintf(filename_ws,255,"/home/ssabidi/Shaukat/WellSSVM/repository/mkl/output/alpha.txt");
	   write_alphas_to_file(filename_ws,alpha,cset.m);
	   
	   /*generate filename*/
	   snprintf(filename_ws,255,"/home/ssabidi/Shaukat/WellSSVM/repository/mkl/output/no_of_ws.txt");
	   write_totws_to_file(filename_ws,cset.m);
	}
  ...
  if(struct_verbosity>=1) {
    ...
    printf("Dual objective value: dval=%.5f\n",
	    alphasum-0.5*modellength*modellength);
	printf("alphasum=%f,modellength=%f,C=%f,viol=%f\n",alphasum,modellength,sparm->C,viol);/*MS-Adding*/
    printf("Primal objective value: pval=%.5f\n",
	    0.5*modellength*modellength+sparm->C*viol);
    printf("Total number of constraints in final working set: %i (of %i)\n",(int)cset.m,(int)totconstraints);
    printf("pval=%.5f,dval=%.5f\n",0.5*modellength*modellength+sparm->C*viol,alphasum-0.5*modellength*modellength);
    /*MS: save total number of constraints in final working set */
    sm->no_ol_ws = (long)cset.m;
    printf("Number of iterations: %d\n",numIt);
    printf("Number of calls to 'find_most_violated_constraint': %ld\n",argmax_count);
    printf("Number of SV: %ld \n",svmModel->sv_num-1);
    printf("Norm of weight vector: |w|=%.5f\n",modellength);
  ...
  print_struct_learning_stats(sample,sm,cset,alpha,sparm);
  print_cset(&cset,alpha); /*MS Adding */
...
/****************************************************************/
void svm_learn_struct_joint_backup(SAMPLE sample, STRUCT_LEARN_PARM *sparm,
			    LEARN_PARM *lparm, KERNEL_PARM *kparm, 
			    STRUCTMODEL *sm, int alg_type)
{
  int         i,j;
  ...
  */
  ...
  do { /* iteratively find and add constraints to working set */
	...
	if(slack > (rhs-lhsXw+0.000001)) {
	/* MS Adding this bit */
	printf(" We will see warning soon....setting violation to zero \n");
	viol = 0.0;
	/* exit(1); */
      }
	/*****************ADDED by MS (End)****************************************************/
...
/*************************************************************/
void svm_learn_struct_joint_original(SAMPLE sample, STRUCT_LEARN_PARM *sparm,
			    LEARN_PARM *lparm, KERNEL_PARM *kparm, 
			    STRUCTMODEL *sm, int alg_type)
{
	/*original function*/
  ...
  */
  ...
  do { /* iteratively find and add constraints to working set */
	...
	if(viol-slack > MAX(epsilon_est/10,sparm->epsilon)) {
	  ...
	  for(j=0;(j<batch_size) || ((j<n)&&(viol-slack<sparm->epsilon));j++) {
	    ...
	    //printf("find most violated constraint for ex:%d\n",i);/*MS-Adding*/
	    ...
	    //printf("rt_viol=%f\n",rt_viol);/*MS-Adding*/
	    //printf("rt_psi=%f\n",rt_psi);/*MS-Adding*/
	    //printf("argmax_count=%ld\n",argmax_count);/*MS-Adding*/
  ...
  if(struct_verbosity>=1) {
    ...
    printf("Dual objective value: dval=%.5f\n",
	    alphasum-0.5*modellength*modellength);
	printf("modellength=%f,C=%f,viol=%f\n",modellength,sparm->C,viol);/*MS-Adding*/
    printf("Primal objective value: pval=%.5f\n",
	    0.5*modellength*modellength+sparm->C*viol);
    printf("Total number of constraints in final working set: %i (of %i)\n",(int)cset.m,(int)totconstraints);
    printf("Number of iterations: %d\n",numIt);
...
void svm_learn_struct_joint_one_slack_dual_cahche_alg(SAMPLE sample, STRUCT_LEARN_PARM *sparm,
			    LEARN_PARM *lparm, KERNEL_PARM *kparm, 
			    STRUCTMODEL *sm, int alg_type)
{
  /* Shaukat: We were using this function before discovering an algorithm that
     doesn't compute cache. This is what we need since we want to store 
     most violated labeling */
...
void svm_learn_struct_joint_for_iterations(SAMPLE sample, STRUCT_LEARN_PARM *sparm,
			    LEARN_PARM *lparm, KERNEL_PARM *kparm, 
			    STRUCTMODEL *sm, int alg_type)
{
  int         i,j;
  ...
  */
  ...
  /*MS: Comment it. We have already gotten it in first iteration*/	
  //init_struct_model(sample,sm,sparm,lparm,kparm); 
  ...
  svmModel=(MODEL *)my_malloc(sizeof(MODEL));
  lparm->epsilon_crit=epsilon;
  svmModel = sm->svm_model; /*MS-Adding: use previous model*/
  ...
  */
  ...
  /* MS: Comment it as we have already obtained initial solution in 
   *     first iteration */
  //svm_learn_optimization(cset.lhs,cset.rhs,cset.m,sizePsi,
  //			 lparm,kparm,NULL,svmModel,alpha);
  ...
  //add_weight_vector_to_linear_model(svmModel); /*MS: We don't need it after first iteration*/
  //sm->svm_model=svmModel; /*MS: We don't need it after first iteration*/
  //sm->w=svmModel->lin_weights; /* short cut to weight vector */ /*MS: We don't need it after first iteration*/
	...
	to the most violated constraint for example ex */
{
  ...
  if(struct_verbosity>=2) rt2=get_runtime();
  (*argmax_count)++;
  //printf("cur_ex: %ld\n",sm->iter_ex_ws);/*MS-Adding*/
  /* MS-Simplifying code: As SLACK RESCALING is not used, we will remove 
     its call to simplify code. See backup for original code */
  ybar=find_most_violated_constraint_marginrescaling(ex->x,ex->y,sm,sparm);
  copy_labels_inside_buffer(&ybar,&(ex->y));/*MS-Adding*/
  //print_buffer_labels(&(ex->y));/*MS-Adding*/
  //printf("line:%d , file: %s , ybar.length = %ld\n",__LINE__,__FILE__,ybar.length);/*MS-Adding*/
  //print_labels(&ybar);/*MS-Adding*/
...
void find_most_violated_constraint_backup(SVECTOR **fydelta, double *rhs, 
				   EXAMPLE *ex, SVECTOR *fycached, long n, 
				   STRUCTMODEL *sm, STRUCT_LEARN_PARM *sparm,
				   double *rt_viol, double *rt_psi, 
				   long *argmax_count)
	...
	run. */
{
  ...
  //printf("exnum:%d\n",exnum); /*MS-Adding*/
  //printf("file:%s,line:%d\n",__FILE__,__LINE__);fflush(stdout);/*MS-Adding*/
  //print_fvec(fydelta);/*MS-Adding*/
  doc_fydelta=create_example(1,0,1,1,fydelta);
  //print_fvec(doc_fydelta->fvec);/*MS-Adding*/
  //printf("file:%s,line:%d\n",__FILE__,__LINE__);fflush(stdout);/*MS-Adding*/
